...
export const Route = createFileRoute("/(main)/snippets/")({
  ...
  const starSnippetMutationOptions = trpc.snippets.star.mutationOptions({
    onMutate: async ({ id, starred }) => {
      ...
      queryClient.setQueryData(["snippets"], (old: Snippet[] | undefined) =>
        old?.map((snippet: Snippet) =>
          snippet.id === id ? { ...snippet, starred } : snippet
        )
      );
  ...
  if (status === "pending") {
    return (
      <div className="flex-1 p-6 pt-2">
        <h1 className="text-2xl mb-6 pl-1">Snippets</h1>
            ...
            >
              <CardHeader className="flex flex-row items-start justify-between">
                ...
                <Button
                  ...
                  onClick={(e) =>
                    handleStarClick(e, snippet.id, !!snippet.starred)
                  }
                >
                  <Star
                    className={`size-4 ${
                      ...
                      {snippet.tags &&
                        ...
                        snippet.tags
                          ...
                          .map((tag: string, index: number) => (
                            <div
                              key={index}
                              className="bg-muted/50 px-2 py-1 rounded whitespace-nowrap overflow-hidden text-ellipsis"
                            >
                              {tag}
                            </div>
                          ))}
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[900px]">
            ...
            <DialogFooter>
              ...
              >
                <Button variant="secondary">Go to Snippet Page</Button>
              </Link>
